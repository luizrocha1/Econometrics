...
```{r message=FALSE, warning=FALSE}
 ...
 install.load::install_load("tidyverse", "GetBCBData", "gridExtra", "zoo", "forecast") 
```
...
df_r <-
  ...
  plot_retorno_selic <- ggplot(data = df_r, aes(x = n,y = selic))+
    geom_line(color = "black")
  
  plot_retorno_ipca <- ggplot(data = df_r, aes(x = n,y = ipca))+
    geom_line(color = "black")
  
  grid.arrange(plot_retorno_cambio, plot_retorno_ipca, plot_retorno_selic)
```

Média não condicional: 
```{r}
 pred_med_df_r <-
    df_r %>% 
      reframe(
...
```{r}
  ...
  erromov_aux <- predit_mov_df_r[1:3] - as.matrix(df_r[13:330,])
  erromov <- erromov_aux$cambio^2 %>% mean()
  
  erromov
```
Pelo cálculo do EQM, temos que a média não condicional dos retornos é melhor procedimento que a médias móveis dos retornos para as previsões. A explicação desse resultado se deve pelo fato de que as séries são aparentemente estacionárias. Portanto, a série corre em torno da média não condicional.

7. b) 

Taxa selic e IPCA para tempo contínuo:
...
A série da selic não parece estacionária pelo plot, mas podemos calcular as médias de dois intervalos de tempo diferentes pra mostrar que as séries não são estacionárias.
...
```
Como a série da selic é não estacionária, precisamos estacionarizar pela diferença dos valores:
...
```{r}
  diff_selic <-
    ...
    rename(diff_selic = selic) %>%
    mutate(n = row_number())
  
  plot_selic_diff <- ggplot(data = diff_selic, aes(x = n, y = diff_selic)) + geom_line(color = "black")
  
  grid.arrange(plot_selic_cont, plot_selic_diff)
```

c) Calcule a taxa de juros real ex-post e repita a análise anterior.
...
```{r}
  ...
  diff_selic_real <-
    ...
    rename(diff_selic_real = selic_real) %>%
    mutate(n = row_number())
    
  plot_selic_real_diff <- ggplot(data = diff_selic_real, aes(x = n, y = diff_selic_real))+
    geom_line(color = "black")
...
```{r}
  ...
  hist(coeficientes, breaks = 15)
```

Como podemos ver a partir do histograma, phi1 estimado continua sendo inconsistente, porém no sentido oposto. phi1 é subestimado.
