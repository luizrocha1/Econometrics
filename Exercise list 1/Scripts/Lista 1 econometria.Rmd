...
As we can see, the cluster-robust standard errors are highly underestimated, it is clear that clustering by region is not a great choice as the number of regions is low. This leads, in a way, to a heteroskedatic regression with only 4 "observations", and the individuals are not so correlated between regions. Clustering by region, in this sense, is just "lowering our information" with no return in terms of bias.
